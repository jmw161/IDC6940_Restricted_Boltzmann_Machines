...
$$
where v<sub>i</sub> and h<sub>j</sub> represent visible and hidden units; a<sub>i</sub> and b<sub>j</sub> are the bia terms of the visible and hidden units; and each w<sub>{i,j}</sub> (weight) element represents the interaction between the visible and hidden units.
...
We use the Fashion MNIST dataset from Zalando Research.. The set includes 70,000 grayscale images of clothing items, 60,000 for training and 10,000 for testing. Each image is 28x28 pixels (784 pixels total). Each pixel has a value associated with it ranging from 0 (white) to 255 (very dark) -- whole numbers only. There are 785 columns in total as one column is dedicated to the label. 
...
train_df = pd.read_csv("/Users/jessicawells/Downloads/archive/fashion-mnist_train.csv")
#test_df = pd.read_csv("/Users/jessicawells/Downloads/archive/fashion-mnist_test.csv")
...
# Initiative the histogram
plt.hist(labels, bins=10, edgecolor='blue')

# Show frequency of labels
plt.xlabel("Class Labels")
plt.ylabel("Frequency")
plt.title("Frequency of Class Labels in X Train")

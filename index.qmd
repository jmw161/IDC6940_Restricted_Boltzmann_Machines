...
<br>
It can be seen the graphical model is undirected. Goodfellow, et al. discuss the expense in drawling samples for most undirected graphical models; however, the RBM allows for block Gibbs sampling (p. 578) where sampling of hidden units simultaneously (etc. for visible). Derivatives are also simplified by the fact that the energy function of the RBM is a linear function of it's parameters.  

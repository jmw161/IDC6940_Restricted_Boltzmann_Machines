...
## Introduction
Restricted Boltzmann Machines are a type of neural network that has been around since the 1980s. They are used for dimensionality reduction/feature extraction which is great for preparing a dataset for unsupervised learning. They also have other applications which will be discussed further later. As a reminder to the reader, machine learning is generally divided into 3 categories: supervised learning (examples: classification tasks, regression), unsupervised learning (examples: clustering, dimensionality reduction, generative modeling), and reinforcement learning (examples: gaming/robotics). So, like most neural networks, RBMs can essentially dip into all 3 categories. Goodfellow, et al. [@Goodfellow-et-al-2016] (p. 577) discuss how RBMs are the classical example of using graphical models for deep learning. The term restricted comes from the fact that in this model, there are no interactions between neurons in the visible layer or between neurons in the hidden layer. Below is a diagram taken from Goodfellow, et al. 
